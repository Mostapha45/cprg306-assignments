...
const ItemList = ({items}) => {
    ...
    let sortedItems = [...items]; //copy the items so theres no changes made
    ...
    const groupedItems = groupByCategory ? groupItemsByCategory([...items]) : null;
            ...
            {!groupByCategory ? (
                <ul className='space-y-4'>
                    {sortedItems.map(item => (
                        <div className=" py-2 ml-2 bg-gray-800 text-white p-4 rounded-lg mb-2 w-1/2" key={item.id}>
                            <Item
                                name={item.name}
